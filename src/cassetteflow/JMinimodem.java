 ...
 * <h3>Core Features:</h3>
 * <ul>
 * <li><b>DSP Engine:</b> Uses a Sliding Window Heterodyne filter (matched
 * filter) for demodulation.</li>
 * <li><b>Soft PLL Sync:</b> Digital Phase-Locked Loop to track signal drift
 * without "snapping" on noise.</li>
 * <li><b>DC Blocker:</b> Removes microphone offset voltage (hum) to improve
 * dynamic range.</li>
 * <li><b>Noise Kill Switch:</b> Drops carrier if framing errors (static) exceed
 * a threshold.</li>
 * <li><b>Speed Measurement:</b> Measures edge deltas to determine instantaneous
 * and average tape playback speed / baud rate.</li>
...
public class JMinimodem {
    ...
    /**
     * Callback interface to receive periodic FSK diagnostic strings from the
     * decoder thread.
     */
    ...
    /**
     * Callback interface to receive structured FSK diagnostic data from the decoder
     * thread.
     */
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if (arg.startsWith("-")) {
                switch (arg) {
                    ...
                    case "--confidence":
                    case "-c":
                        cfg.confidenceThreshold = Double.parseDouble(args[++i]);
                        break;
                    case "--mark":
                        cfg.freqMark = Double.parseDouble(args[++i]);
                        break;
                    case "--space":
                        cfg.freqSpace = Double.parseDouble(args[++i]);
                        break;
                    default:
                        break;
                }
        ...
        if (!positional.isEmpty())
            applyBaudMode(cfg, positional.get(0));
    ...
    private static void runCliTransmit(Config cfg) throws Exception {
        ...
        if (cfg.audioFile != null) {
            ...
            if (!cfg.quiet)
                System.err.println("Wrote " + cfg.audioFile.getName());
     ...
     * @param output Audio destination (Raw PCM). If NULL, plays to default
     *               Speakers.
     */
     ...
     */
    private static double emit(Config c, double freq, double samples, double phase, OutputStream out,
            SourceDataLine line) throws IOException {
     ...
     * Includes High Sensitivity DSP, Digital PLL, Noise Rejection Kill Switch, and
     * Speed Measurement.
     * 
    ...
    public static void receive(Config cfg, AudioInputStream audioSrc, OutputStream textOut) throws Exception {
        if (!cfg.quiet)
            System.err.printf("### RX: %.0f baud @ %.0fHz (High Sensitivity)\n", cfg.baudRate, cfg.sampleRate);

        // --- DSP Initialization ---
        double samplesPerBit = cfg.sampleRate / cfg.baudRate;
        long lastDiagTime = System.currentTimeMillis();
        long lastDiagSample = 0;

        // Use 1.0 (Full Width) window
        double windowWidth = 1.0;

        SlidingFilter filterMark = new SlidingFilter(cfg.invert ? cfg.freqSpace : cfg.freqMark, cfg.sampleRate,
                (int) (samplesPerBit * windowWidth));
        SlidingFilter filterSpace = new SlidingFilter(cfg.invert ? cfg.freqMark : cfg.freqSpace, cfg.sampleRate,
                (int) (samplesPerBit * windowWidth));
        ...
        long sampleCount = 0;
        long lastStartBitSample = 0;
        long burstStartSample = 0;
        int burstCharCount = 0;
        double smoothedSamplesPerBit = samplesPerBit;
        ...
        while ((bytesRead = audioSrc.read(buf)) != -1) {
            ...
            if (cfg.resetDiagnostics) {
                ...
                sideBLineCount = 0;
                sideBErrors = 0;
                smoothedSamplesPerBit = samplesPerBit;
                lastMeasuredBaud = 0.0;
                sampleCount = 0;
                lastEdgeSample = 0;
                lastStartBitSample = 0;
                burstStartSample = 0;
                burstCharCount = 0;
                lastDiagSample = 0;
            ...
            for (int i = 0; i < bytesRead; i += 2) {
                ...
                if (now - lastDiagTime >= 1000
                        || (cfg.sampleRate > 0 && sampleCount - lastDiagSample >= cfg.sampleRate)) {
                    lastDiagTime = now;
                    lastDiagSample = sampleCount;
                    if ((cfg.diagListener != null || cfg.diagDataListener != null) && carrier) {
                        ...
                        sb.append(String.format("Baud: %d (%c%.1f%%) | SNR: %.2f | Sig: %d%%\n",
                                (int) lastMeasuredBaud, sign, Math.abs(speedOffset), conf,
                                (int) (signalLevel * 100.0)));
                        ...
                        // 6. Side A Stats
                        float saPerc = (sideALineCount > 0) ? ((float) sideAErrors * 100.0f / (float) sideALineCount)
                                : 0.0f;
                        sb.append(String.format("Side A: %d/%d (%.2f%%)\n", sideAErrors, sideALineCount, saPerc));

                        // 7. Side B Stats
                        float sbPerc = (sideBLineCount > 0) ? ((float) sideBErrors * 100.0f / (float) sideBLineCount)
                                : 0.0f;
                ...
                double maxMag = isMark ? mMag : sMag;
                double minMag = isMark ? sMag : mMag;
                double snrRatio = maxMag / (minMag + 0.02);

                if (total > cfg.noiseFloor && (conf > 0.5 || snrRatio >= 1.20)) {
                    if (carrierCounter < samplesPerBit * 2)
                        carrierCounter++;
                } else {
                    if (carrierCounter > -samplesPerBit * 2)
                        carrierCounter--;
                ...
                // Frame-Aligned Multi-Byte Burst Baud Rate Measurement on Start-Bit Falling Edges (Mark -> Space)
                if (isMark != lastBitMark) {
                    if (state == STATE_IDLE && !isMark) {
                        long currentSample = sampleCount;
                        if (lastStartBitSample > 0) {
                            long delta = currentSample - lastStartBitSample;
                            double bits = (double) delta / samplesPerBit;
                            int kBits = (int) (bits + 0.5);

                            // Standard 10-bit UART frame (1 start + 8 data + 1 stop = 10 bits), allow up to 14 bits for small inter-byte gaps
                            if (kBits >= 9 && kBits <= 14) {
                                double instBaud = ((double) kBits * cfg.sampleRate / (double) delta);
                                if (instBaud >= cfg.baudRate * 0.85 && instBaud <= cfg.baudRate * 1.15) {
                                    if (lastMeasuredBaud < 1.0) {
                                        lastMeasuredBaud = instBaud;
                                    } else {
                                        lastMeasuredBaud = (lastMeasuredBaud * 0.95) + (instBaud * 0.05);
                                    }
                                }
                                burstCharCount++;

                                // Multi-byte burst regression for ultra-high accuracy over consecutive characters
                                if (burstCharCount >= 4 && burstStartSample > 0) {
                                    long burstDelta = currentSample - burstStartSample;
                                    double totalBits = (double) burstCharCount * 10.0;
                                    double burstBaud = (totalBits * cfg.sampleRate / (double) burstDelta);
                                    if (burstBaud >= cfg.baudRate * 0.90 && burstBaud <= cfg.baudRate * 1.10) {
                                        lastMeasuredBaud = (lastMeasuredBaud * 0.85) + (burstBaud * 0.15);
                                    }
                                }
                            } else {
                                // Gap between bursts
                                burstStartSample = currentSample;
                                burstCharCount = 1;
                            }
                        } else {
                            burstStartSample = currentSample;
                        ...
                        lastStartBitSample = currentSample;

                        // Start bit transition verified
                        timer = samplesPerBit * 0.5;
                        state = STATE_VERIFY;
                    }

                    // Secondary fallback for continuous tones/preamble edges (k >= 6 bit intervals only)
                    if (lastEdgeSample > 0 && burstCharCount < 3) {
